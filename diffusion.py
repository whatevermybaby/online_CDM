...
class GaussianDiffusion(nn.Module):
    def __init__(self, model, image_size, timesteps=1000, beta_schedule='cosine',condi_coef=1,):
        super().__init__()
        self.model = model
        self.image_size = image_size
        self.num_timesteps = timesteps
        self.condi_coef=condi_coef
    ...
    def p_mean_variance(self, x, t, classes,w):
        pred_noise = (1+w)*self.model(x, t, classes)-w*self.model(x, t, torch.zeros_like(classes)-10000)
    ...
    @torch.no_grad()
    def p_sample(self, x, t, classes,w):
        batched_times = torch.full((x.shape[0],), t, device=x.device, dtype=torch.long)
        model_mean, model_log_variance = self.p_mean_variance(x=x, t=batched_times, classes=classes,w=w)
        noise = torch.randn_like(x) if t > 0 else 0.

        return model_mean + (0.5 * model_log_variance).exp() * noise

    @torch.no_grad()
    def sample(self, classes,w):
        img = torch.randn((classes.shape[0], self.image_size), device=self.betas.device)
        for t in range(self.num_timesteps - 1, -1, -1):
            img = self.p_sample(img, t, classes,w)
        magnitude = torch.norm(img, dim=1)
    ...
    def p_losses(self, x_start, t, classes):
        ...
        p=0.1;pas=random.random()
        if pas < p:
            label=torch.zeros_like(classes)-10000
        else:
            label=classes
        # return F.mse_loss((1+w)*self.model(x, t, classes)-w*self.model(x, t, torch.zeros_like(classes)-10000), noise)
        return F.mse_loss(self.model(x, t, label), noise)
